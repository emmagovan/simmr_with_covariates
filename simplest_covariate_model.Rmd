---
title: "simplest covariate model"
author: "Emma Govan"
date: "2023-04-06"
...
# Maths for simplest covariate model
...
This has $c_0, d_0, \alpha_0, \beta_0$ etc


$$\log{q_\lambda}(\theta) = \log(q(f)q(\tau))$$

Which uses the new iterative value of 
$$\mu_\alpha, \Sigma_\alpha, \mu_\beta, \sigma_\beta, c$$, and $d$ generated in every iteration of the model

sim_theta will also have to be specified, this generates new values of $\tau,\alpha$ and $\beta$ every iteration

Everything else is already done in the generic simmr functions.

$\theta = \alpha, \beta, \tau$

$\lambda = \mu_\alpha, \Sigma_\alpha, \mu_\beta, \Sigma_\beta,  c, d$
...
x_i0 <- sex
n <- nrow(y)
mu_alpha_zero <- c(rep(1, K))
mu_beta_zero <- c(rep(1, K))
...
sim_theta <- function(S, lambda) {
  ...
  theta <- cbind(
   ...
   matrix(rep(0, K*S), ncol = K, nrow = S), #t(rMVNormC(S, mu = mean_beta, U = chol_prec_beta)), #
...
h <- function(theta) {
  ...
  hold<-0
   ...
   hold = hold + sum(dnorm(y[i, 1],
    ...
    + sum(dnorm(y[i, 2],
                   mean = sum(p[i,] * q * (mu_kj[, 2]+mu_c[,2]))/sum(p[i,]*q),
                   sd = sqrt(sum(p[i,]^2 * q^2 * (sigma_kj[, 2]^2+sigma_c[,2]^2))/sum(p[i,]^2*q^2) + 1 / theta[2 *K + 2]),
                   log = TRUE
  ))
    } 
...
# Algorithm ---------------------------------------------------------------
lambda <- run_VB(lambda = c(rep(1, K), rep(1, ((K*(K+1) +K)+2*n_isotopes)))) # Starting value of lambda
